"""
╔════════════════════════════════════════════════════════════╗
║  MindLink — Ingestão Oracle Autonomous Database            ║
║  Sprint 3: dados PySUS reais + tabelas analíticas           ║
╚════════════════════════════════════════════════════════════╝

Carrega no Oracle ADB os artefatos gerados pelo pipeline:
  mindlink_internacoes_sp.parquet      → MINDLINK_INTERNACOES
  historico_indicadores.csv            → MINDLINK_INDICADORES
  projecao_demencia_2050.csv           → MINDLINK_PROJECAO
  territorio_scores.csv                → MINDLINK_TERRITORIO
  comorbidades.csv                     → MINDLINK_COMORBIDADES
  comparativo_filtro_demencia.csv      → MINDLINK_COMPARATIVO_FILTRO
  capacidade_cnes_sp.csv               → MINDLINK_CAPACIDADE_CNES

Pré-requisitos:
    pip install oracledb pandas pyarrow python-dotenv
"""

from __future__ import annotations

import math
import os
import warnings

import pandas as pd

warnings.filterwarnings("ignore")

try:
    from dotenv import load_dotenv
    load_dotenv()
except ImportError:
    pass

try:
    import oracledb
except ImportError as exc:
    raise ImportError("Execute: pip install oracledb") from exc

ORACLE_USER = os.getenv("ORACLE_USER", "ADMIN")
ORACLE_PWD = os.getenv("ORACLE_PASSWORD", "")
ORACLE_DSN = os.getenv("ORACLE_DSN", "")
WALLET_PATH = os.getenv("ORACLE_WALLET_PATH", "")
WALLET_PWD = os.getenv("ORACLE_WALLET_PASSWORD", "")

OUTPUT_PROC = "data/processed/"
BATCH_SIZE = 500

ARQ = {
    "internacoes": os.path.join(OUTPUT_PROC, "mindlink_internacoes_sp.parquet"),
    "historico": os.path.join(OUTPUT_PROC, "historico_indicadores.csv"),
    "projecao": os.path.join(OUTPUT_PROC, "projecao_demencia_2050.csv"),
    "territorio": os.path.join(OUTPUT_PROC, "territorio_scores.csv"),
    "comorbidades": os.path.join(OUTPUT_PROC, "comorbidades.csv"),
    "comparativo": os.path.join(OUTPUT_PROC, "comparativo_filtro_demencia.csv"),
    "capacidade": os.path.join(OUTPUT_PROC, "capacidade_cnes_sp.csv"),
}

DDL = [
    """
    CREATE TABLE MINDLINK_INTERNACOES (
        id                      NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        n_aih                   VARCHAR2(30),
        cid_principal           VARCHAR2(10),
        cid_secundario          VARCHAR2(10),
        cid_demencia_detectado  VARCHAR2(5),
        cid_grupo_principal     VARCHAR2(5),
        cid_label               VARCHAR2(80),
        demencia_em             VARCHAR2(30),
        demencia_principal      NUMBER(1),
        demencia_secundaria     NUMBER(1),
        municipio_residencia    VARCHAR2(10),
        municipio_internacao    VARCHAR2(10),
        ano                     NUMBER(4),
        mes                     NUMBER(2),
        dias_permanencia        NUMBER(8,2),
        valor_total             NUMBER(15,2),
        idade                   NUMBER(3),
        faixa_etaria            VARCHAR2(10),
        sexo                    VARCHAR2(3),
        sexo_label              VARCHAR2(15),
        cnes_hospital           VARCHAR2(20),
        obito                   NUMBER(1)
    )
    """,
    """
    CREATE TABLE MINDLINK_INDICADORES (
        ano                     NUMBER(4) PRIMARY KEY,
        internacoes             NUMBER(12),
        internacoes_principal   NUMBER(12),
        internacoes_secundaria  NUMBER(12),
        valor_medio             NUMBER(15,2),
        perm_media              NUMBER(8,2),
        obitos                  NUMBER(12),
        letalidade_pct          NUMBER(8,2)
    )
    """,
    """
    CREATE TABLE MINDLINK_PROJECAO (
        ano                     NUMBER(4) PRIMARY KEY,
        pop_60_mais_milhoes     NUMBER(8,2),
        taxa_por_100k           NUMBER(8,2),
        fonte_taxa              VARCHAR2(300),
        internacoes_proj        NUMBER(12),
        custo_a_congelado       NUMBER(18,2),
        custo_b_parcial         NUMBER(18,2),
        custo_c_pleno           NUMBER(18,2),
        eh_projecao             NUMBER(1)
    )
    """,
    """
    CREATE TABLE MINDLINK_TERRITORIO (
        id                      NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        uf                      VARCHAR2(2),
        nome                    VARCHAR2(120),
        municipio_residencia    VARCHAR2(10),
        internacoes             NUMBER(12),
        perm_media              NUMBER(8,2),
        valor_total             NUMBER(18,2),
        obitos                  NUMBER(12),
        crescimento_pct         NUMBER(8,2),
        score                   NUMBER(5),
        nivel                   VARCHAR2(20)
    )
    """,
    """
    CREATE TABLE MINDLINK_COMORBIDADES (
        cid_secundario          VARCHAR2(10) PRIMARY KEY,
        descricao               VARCHAR2(120),
        internacoes             NUMBER(12)
    )
    """,
    """
    CREATE TABLE MINDLINK_COMPARATIVO_FILTRO (
        modo_filtro             VARCHAR2(40) PRIMARY KEY,
        internacoes             NUMBER(12),
        percentual_total        NUMBER(10,4)
    )
    """,
    """
    CREATE TABLE MINDLINK_CAPACIDADE_CNES (
        id                      NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        ano                     NUMBER(4),
        mes                     NUMBER(2),
        uf                      VARCHAR2(2),
        municipio               VARCHAR2(10),
        cnes                    VARCHAR2(20),
        nome_estabelecimento    VARCHAR2(200),
        tipo_unidade            VARCHAR2(120),
        leitos_existentes       NUMBER(12),
        leitos_sus              NUMBER(12),
        uti_existente           NUMBER(12),
        uti_sus                 NUMBER(12)
    )
    """,
]

VIEWS = [
    """
    CREATE OR REPLACE VIEW VW_INTERNACOES_AGRUPADAS AS
    SELECT ano,
           cid_demencia_detectado AS cid_grupo,
           cid_label,
           faixa_etaria,
           demencia_em,
           COUNT(*) AS internacoes,
           SUM(valor_total) AS custo_total_rs,
           ROUND(AVG(valor_total), 2) AS custo_medio_rs,
           ROUND(AVG(dias_permanencia), 1) AS media_dias_perm,
           SUM(obito) AS total_obitos
    FROM MINDLINK_INTERNACOES
    GROUP BY ano, cid_demencia_detectado, cid_label, faixa_etaria, demencia_em
    """,
    """
    CREATE OR REPLACE VIEW VW_KPIS_EXECUTIVOS AS
    SELECT ano,
           internacoes_proj AS internacoes,
           ROUND(custo_a_congelado/1e6, 2) AS custo_cenario_a_milhoes,
           ROUND(custo_c_pleno/1e6, 2) AS custo_cenario_c_milhoes,
           taxa_por_100k,
           pop_60_mais_milhoes,
           eh_projecao
    FROM MINDLINK_PROJECAO
    """,
    """
    CREATE OR REPLACE VIEW VW_PRESSAO_ASSISTENCIAL AS
    SELECT t.uf,
           t.nome,
           t.municipio_residencia,
           t.internacoes,
           c.leitos_sus,
           CASE WHEN c.leitos_sus > 0 THEN ROUND(t.internacoes / c.leitos_sus, 4) END AS internacoes_por_leito_sus,
           t.score,
           t.nivel
    FROM MINDLINK_TERRITORIO t
    LEFT JOIN (
        SELECT municipio, SUM(leitos_sus) AS leitos_sus
        FROM MINDLINK_CAPACIDADE_CNES
        GROUP BY municipio
    ) c ON c.municipio = t.municipio_residencia
    """,
]

COMMENTS = [
    "COMMENT ON TABLE MINDLINK_INTERNACOES IS 'Internações SIH/SUS com demência detectada no diagnóstico principal ou secundário. Fonte: DATASUS via PySUS.'",
    "COMMENT ON COLUMN MINDLINK_INTERNACOES.demencia_em IS 'Indica se a demência foi encontrada em DIAG_PRINC, DIAG_SECUN ou ambos.'",
    "COMMENT ON TABLE MINDLINK_COMPARATIVO_FILTRO IS 'Compara volume de internações capturado por DIAG_PRINC, DIAG_SECUN e DIAG_PRINC + DIAG_SECUN.'",
    "COMMENT ON TABLE MINDLINK_CAPACIDADE_CNES IS 'Capacidade hospitalar extraída do CNES para cálculo de pressão assistencial.'",
]


def conectar() -> "oracledb.Connection":
    if WALLET_PATH and os.path.isdir(WALLET_PATH):
        print(f"[Oracle] Conectando com Wallet: {WALLET_PATH}")
        conn = oracledb.connect(
            user=ORACLE_USER,
            password=ORACLE_PWD,
            dsn=ORACLE_DSN,
            config_dir=WALLET_PATH,
            wallet_location=WALLET_PATH,
            wallet_password=WALLET_PWD,
        )
    else:
        print("[Oracle] Conectando sem Wallet (TCP direto)...")
        conn = oracledb.connect(user=ORACLE_USER, password=ORACLE_PWD, dsn=ORACLE_DSN)
    print(f"[Oracle] ✅ Conectado — versão {conn.version}")
    return conn


def criar_estrutura(conn: "oracledb.Connection") -> None:
    with conn.cursor() as cur:
        for ddl in DDL:
            nome = ddl.strip().split()[2]
            try:
                cur.execute(ddl)
                print(f"[DDL] Tabela {nome} criada.")
            except oracledb.DatabaseError as e:
                if "ORA-00955" in str(e):
                    print(f"[DDL] Tabela {nome} já existe (ok).")
                else:
                    raise
        for view_sql in VIEWS:
            nome_view = view_sql.strip().split()[4]
            cur.execute(view_sql)
            print(f"[DDL] View {nome_view} criada/substituída.")
        for comment in COMMENTS:
            try:
                cur.execute(comment)
            except oracledb.DatabaseError:
                pass
    conn.commit()
    print("[DDL] ✅ Estrutura criada.\n")


def limpar_tabela(conn: "oracledb.Connection", tabela: str) -> None:
    with conn.cursor() as cur:
        try:
            cur.execute(f"DELETE FROM {tabela}")
            conn.commit()
        except oracledb.DatabaseError:
            pass


def inserir_df(df: pd.DataFrame, tabela: str, colunas: list[str], conn: "oracledb.Connection") -> None:
    cols = [c for c in colunas if c in df.columns]
    if not cols:
        print(f"  [{tabela}] ⚠ nenhuma coluna esperada — pulando.")
        return

    df_ins = df[cols].copy().where(pd.notnull(df[cols]), None)
    cols_sql = ", ".join(df_ins.columns).upper()
    ph = ", ".join([f":{i+1}" for i in range(len(df_ins.columns))])
    sql = f"INSERT INTO {tabela} ({cols_sql}) VALUES ({ph})"
    rows = [tuple(r) for r in df_ins.itertuples(index=False)]
    total = len(rows)

    limpar_tabela(conn, tabela)
    with conn.cursor() as cur:
        for b in range(math.ceil(total / BATCH_SIZE) if total else 0):
            cur.executemany(sql, rows[b * BATCH_SIZE:(b + 1) * BATCH_SIZE])
            print(f"  [{tabela}] {min((b + 1) * BATCH_SIZE, total):,}/{total:,}", end="\r")
    conn.commit()
    print(f"\n  ✅ {total:,} registros → {tabela}")


def ingerir(chave: str, tabela: str, colunas: list[str], conn: "oracledb.Connection", leitor) -> None:
    caminho = ARQ[chave]
    if not os.path.exists(caminho):
        print(f"  [{tabela}] ⚠ {os.path.basename(caminho)} ausente — etapa pulada.")
        return
    df = leitor(caminho)
    inserir_df(df, tabela, colunas, conn)


def main() -> None:
    conn = conectar()
    criar_estrutura(conn)

    ingerir("internacoes", "MINDLINK_INTERNACOES", [
        "n_aih", "cid_principal", "cid_secundario", "cid_demencia_detectado", "cid_grupo_principal",
        "cid_label", "demencia_em", "demencia_principal", "demencia_secundaria", "municipio_residencia",
        "municipio_internacao", "ano", "mes", "dias_permanencia", "valor_total", "idade", "faixa_etaria",
        "sexo", "sexo_label", "cnes_hospital", "obito",
    ], conn, pd.read_parquet)

    ingerir("historico", "MINDLINK_INDICADORES", [
        "ano", "internacoes", "internacoes_principal", "internacoes_secundaria", "valor_medio",
        "perm_media", "obitos", "letalidade_pct",
    ], conn, pd.read_csv)

    ingerir("projecao", "MINDLINK_PROJECAO", [
        "ano", "pop_60_mais_milhoes", "taxa_por_100k", "fonte_taxa", "internacoes_proj",
        "custo_a_congelado", "custo_b_parcial", "custo_c_pleno", "eh_projecao",
    ], conn, lambda p: pd.read_csv(p).rename(columns={
        "custo_A_congelado": "custo_a_congelado",
        "custo_B_parcial": "custo_b_parcial",
        "custo_C_pleno": "custo_c_pleno",
    }))

    ingerir("territorio", "MINDLINK_TERRITORIO", [
        "uf", "nome", "municipio_residencia", "internacoes", "perm_media", "valor_total", "obitos",
        "crescimento_pct", "score", "nivel",
    ], conn, pd.read_csv)

    ingerir("comorbidades", "MINDLINK_COMORBIDADES", ["cid_secundario", "descricao", "internacoes"], conn, pd.read_csv)
    ingerir("comparativo", "MINDLINK_COMPARATIVO_FILTRO", ["modo_filtro", "internacoes", "percentual_total"], conn, pd.read_csv)
    ingerir("capacidade", "MINDLINK_CAPACIDADE_CNES", [
        "ano", "mes", "uf", "municipio", "cnes", "nome_estabelecimento", "tipo_unidade",
        "leitos_existentes", "leitos_sus", "uti_existente", "uti_sus",
    ], conn, pd.read_csv)

    conn.close()
    print("\n✅ Pipeline de ingestão Oracle concluído!")


if __name__ == "__main__":
    main()
